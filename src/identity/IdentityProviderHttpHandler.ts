...
import { readableToDict } from '../util/StreamUtil';
...
export interface IdentityProviderHttpHandlerArgs extends BaseHttpHandlerArgs {
  // BaseHttpHandlerArgs fields aren't copied by Components.js
...
export class IdentityProviderHttpHandler extends BaseHttpHandler {
  ...
  private async handleInteractionResult(operation: Operation, request: HttpRequest,
    ...
    if (result.type === 'complete') {
      ...
      const details = await readableToDict(response.data!);

      // Add the input data to the JSON response;
      if (operation.body) {
        details.prefilled = await readableToDict(operation.body.data);
